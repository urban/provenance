...
print_help() {
  echo "Usage: scripts/ralph/loop.sh [input-file-path] [options]"
  echo ""
  ...
  echo "                                    Default: scripts/ralph/CHECKLIST.md"
  echo ""
  echo "Options:"
...
if [[ "${#positionals[@]}" -gt 1 ]]; then
  ...
  echo "Usage: scripts/ralph/loop.sh [input-file-path] [options]" >&2
  exit 1
fi
